...
This plugin comes with a quality profile containing not only the aforementioned rules, but also rules from other plugins that are vital to ensure optimal code quality. Make sure to import the following plugins into your SonarQube instance, so that you are able to fully use this profile:

* https://github.com/checkstyle/sonar-checkstyle/releases/tag/4.27[Checkstyle]
* https://github.com/QualInsight/qualinsight-plugins-sonarqube-smell/releases/tag/qualinsight-plugins-sonarqube-smell-4.0.0[Code Smells]
* https://github.com/spotbugs/sonar-findbugs/releases/tag/3.11.1[FindBugs]
* https://github.com/jensgerdes/sonar-pmd/releases/tag/3.2.1[PMD]

You can either associate this profile with certain projects or set it as default to make it active for every project. If you want to make changes to the profile, just create a copy of it where you can then include or exclude rules from it.
